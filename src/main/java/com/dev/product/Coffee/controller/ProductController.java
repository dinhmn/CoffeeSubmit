...
//    }
    @PostMapping("/product")
    public ResponseEntity<?> createProduct(@RequestBody ProductDTO productDTO, @RequestBody'' MultipartFile multipartFile) throws Exception {
        Long id = productDTO.getCategoryId();
        CategoriesEntity categoriesEntity = categoriesService.getCategories(id);
        
        ProductEntity productEntity = productService.createProduct(ProductEntity.from(productDTO), multipartFile, categoriesEntity);
