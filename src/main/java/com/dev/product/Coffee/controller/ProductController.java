...
public class ProductController {
  ...
  public ResponseEntity<List<ProductDTO>> getAllProductsByPrice(
    ...
    List<ProductDTO> productDTOList =
        productEntityList.stream().map(ProductDTO::fromTo).collect(Collectors.toList());

    return new ResponseEntity<>(productDTOList, HttpStatus.OK);
  }

  @GetMapping("/product/search")
  public ResponseEntity<List<ProductDTO>> getAllProductsByTitle(
      @RequestParam(value = "title") String title) {
    List<ProductEntity> productEntityList =
        productService.selectProdcutByTitle(title, Sort.by("price"));
    List<ProductDTO> productDTOList =
        productEntityList.stream().map(ProductDTO::fromTo).collect(Collectors.toList());
    return new ResponseEntity<>(productDTOList, HttpStatus.OK);
  }
  ...
  public ResponseEntity<List<ProductDTO>> getAllProductsByQuantity(
      @PathVariable(value = "quantity") Long quantity) {
    List<ProductEntity> productEntityList =
        productService.selectProductByQuantity(quantity);
    List<ProductDTO> productDTOList =
        productEntityList.stream().map(ProductDTO::fromTo).collect(Collectors.toList());
    return new ResponseEntity<>(productDTOList, HttpStatus.OK);
  }
  ...
  public ResponseEntity<List<ProductDTO>> getAllProductsByPriceMinMax(@PathVariable String max,
                                                                      @PathVariable String min) {
    List<ProductEntity> productEntityList =
        productService.selectProdcutByPriceRange(Long.parseLong(min), Long.parseLong(max));
    List<ProductDTO> productDTOList =
        productEntityList.stream().map(ProductDTO::fromTo).collect(Collectors.toList());
    return new ResponseEntity<>(productDTOList, HttpStatus.OK);
  }
